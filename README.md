...
- **DataBricks Free Version:** Encontramos limitaciones trabajando con la version gratuita de DataBricks. Por ejemplo, no pudimos guardar las corridas de Optuna en disco para tener persistencia de las iteraciones realizadas, el tiempo limite de uso diario limitaba la cantidad de iteraciones posibles. Tampoco pudimos generar un endpoint del modelo en Spark, porque es necesario correrlo en Java, cosa que no se puede en la version gratuita.
- **Trabajar con datos sinteticos:** Los datasets de transacciones bancarias tienen problemas de etica, por lo cual trabajamos con uno sintetico. Es posible que al generar los fraudes intencionalmente, el modelo haya obtenidio una muy buena performance por esta razón. Con datasets reales, es posible el feature engineering tendría que tener mayor trabajo y rigurosidad.
